...
import com.jlkf.oidemo.other.utils.DensityUtils;

import java.util.ArrayList;

/***
 * * @description api 9以上的wheelview
 * * 由 马君 创建于 2016年03月09日 16:42
 ***/
...
public class WheelView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for(int i=startItemPos,j=0;i<startItemPos+showSize+2&&i<data.size();j++,i++){
        ...
        if(changeListener!=null){
            if(scrollY<0){
                if(Math.abs(scrollY%itemHeight)<itemHeight/2){      //滑动一半时通知地址
                    changeListener.onScroll(scrollY,startItemPos+(showSize-1)/2);
                }else{
                    changeListener.onScroll(scrollY,startItemPos+(showSize+1)/2);
                }
            }else{
                if(scrollY>itemHeight*(showSize-1)/2-itemHeight/2){      //滑动一半时通知地址
                    changeListener.onScroll(scrollY,0);
                }else{
                    if(Math.abs(scrollY%itemHeight)<itemHeight/2){      //滑动一半时通知地址
                        changeListener.onScroll(scrollY,startItemPos+(showSize-1)/2);
                    }else{
    ...
    public void setCenterItemByBackData(Object backData){
        int size=data.size();
        for(int i=0;i<size;i++){
            if(backData.equals(data.get(i).backData)){
                cacheNowItem=i;
                invalidate();
                return;
            }
    ...
    public Object getCenterItem(){
        if(cacheNowItem>=0&&cacheNowItem<dataSize){
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()){
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                Log.e("y-------",y+"   "+"   "+ dy+"");
                pretendScrollY(dy);
                Log.e("scrolly-------",scrollY+"  " +itemHeight*dataSize);
...
//            mScroller.springBack(0,(int)scrollY,0,0,minScrollY,maxScrollY);
    ...
    public interface OnChangeListener{
        void onScroll(float scrollY, int centerPos);
    }
}
